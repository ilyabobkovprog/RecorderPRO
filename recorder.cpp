...
void RecordLogic(int delay, std::string folderPath, ma_device_id *pSelectedId)
{
    ...
    std::string finalPath = folderPath + "/final_record.mp4";

    ma_encoder_config encCfg = ma_encoder_config_init(ma_encoding_format_wav, ma_format_s16, 1, 16000);
    ma_encoder_init_file(audioPath.c_str(), &encCfg, &encoder);
    ...
    std::string cmd = "ffmpeg -y -loglevel quiet -f x11grab -video_size 1920x1080 -i :0.0 -c:v libx264 -preset ultrafast " + videoPath;
...
public:
    RecorderFrame() : wxFrame(NULL, wxID_ANY, "RECORDER PRO", wxDefaultPosition, wxSize(430, 315))
    {
        ...
        settingsBox->Add(new wxStaticText(m_panel, wxID_ANY, wxString::FromUTF8("Микрофон:")), 0, wxALL, 5);
        ...
        m_linkOpenFolder->SetForegroundColour(wxColour(0, 0, 255));
        m_linkOpenFolder->SetVisitedColour(wxColour(0, 0, 200));
        ...
        mainSizer->Add(hotkeyHint, 0, wxALIGN_CENTER | wxTOP | wxBOTTOM, 5); // Подсказка под кнопками
        mainSizer->AddSpacer(5);                                             // Отступ между подсказкой и статусом
...
private:
    ...
    void StartAction()
    {
        ...
        {
            pId = &m_devices[sel - 1].id;
        }

        m_currentFolderPath = wxDateTime::Now().Format("rec_%Y%m%d_%H%M%S");
        g_IsRecording = true;
        g_IsPaused = false;
        ...
        std::thread(RecordLogic, m_spinDelay->GetValue(), m_currentFolderPath.ToStdString(), pId).detach();
    }
    ...
    void RefreshDevices()
    {
        ...
        m_devices.clear();
        m_deviceChoice->Append(wxString::FromUTF8("Устройство по умолчанию"));
        ...
        if (ma_context_init(NULL, 0, NULL, &context) == MA_SUCCESS)
        {
            ...
            if (ma_context_get_devices(&context, NULL, NULL, &pCaptureInfos, &captureCount) == MA_SUCCESS)
            {
                ...
                {
                    m_devices.push_back({pCaptureInfos[i].id, pCaptureInfos[i].name});
                    m_deviceChoice->Append(wxString::FromUTF8(pCaptureInfos[i].name));
                }
            }
    ...
    void StartGlobalHotkeyThread()
    {
        std::thread([this]()
            ...
            if (!dpy) return;
            int keycodeToggle = XKeysymToKeycode(dpy, XK_F10);
            int keycodePause = XKeysymToKeycode(dpy, XK_F11);
            Window root = DefaultRootWindow(dpy);
            XGrabKey(dpy, keycodeToggle, AnyModifier, root, True, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, keycodePause, AnyModifier, root, True, GrabModeAsync, GrabModeAsync);
            XEvent ev;
            while (true) {
                XNextEvent(dpy, &ev);
                if (ev.type == KeyPress) {
                    if (ev.xkey.keycode == keycodeToggle) {
                        this->GetEventHandler()->CallAfter([this]() { 
                            if (!g_IsRecording) StartAction(); else StopAction(); 
                        });
                    } else if (ev.xkey.keycode == keycodePause) {
                        this->GetEventHandler()->CallAfter([this]() { PauseAction(); });
                    }
                }
            } })
            .detach();
...
public:
    virtual bool OnInit()
    {
        ...
        frame->Show();
        frame->Centre(); // Добавьте эту строку
        return true;
    }
};
